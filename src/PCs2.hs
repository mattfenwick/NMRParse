...
instance Monad (Either a) where
  return = Right
  fail = error "don't use fail with this monad"
  (Right r) >>= f = f r
  (Left l) >>= _ = Left l
...
-- data Parser a b = Parser {f :: ([a], Integer) -> Either (Failure a) (([a], Integer), b)}
...
-- succeed val = Parser (\inp -> Right (inp, val))
...
pseq l r inp = l inp >>= 
  \((toks1, ct1), res1) -> r (toks1, ct1) >>= 
  \((toks2, ct2), res2) -> return ((toks2, ct2), (res1, res2))
