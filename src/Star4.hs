...
identifier = literal '_' <|> \_ ->
  (some $ pnone " \t\n\r\f\v") <|> \cs ->
  succeed (Identifier cs)
...
pLoop = literal Loop <|>
  ...
  \is -> many val <|>
  \vs -> literal Stop <|>
  \_ -> succeed (PLoop is vs)

-- which way is better?
-- pLoop = literal Loop <|> \_ ->
--   many ident <|> \is ->
--   many val <|> \vs -> 
--   literal Stop <|> \_ ->
--   succeed (PLoop is vs) 
